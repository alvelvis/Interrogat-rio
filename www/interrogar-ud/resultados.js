var translations = {
	...
	'Depois, <a href="#" onclick="saveOpenAIKey()">clique aqui</a> para salvar a sua chave de API no Interrogatório.': {
		'en-US': 'Then, <a href="#" onclick="saveOpenAIKey()">click here</a> to integrate your API key to Interrogatório.'
	},
	'Chave API salva com sucesso.': {
		'en-US': 'API key was saved successfully.'
	},
	'O código de busca gerado pela IA será baixado na sua máquina, lembre-se de revisá-lo se necessário.': {
		'en-US': 'The query script generated by the AI will be downloaded in your machine, remember to revise it if needed.'
	},
	'Escreva seu código de busca Python': {
		'en-US': 'Write your Python query script'
	},
	...
	'Para experientes: Utilize algum dos padrões de busca abaixo': {
		'en-US': 'For the more experienced: Use one of the query patterns below'
	},
	'Critério 1: Regex': {
	...
	'Simplificação: os padrões de busca agora são Regex, Python Simplificado e código de busca em Python': {
		'en-US': 'Simplification: the querying patterns are now Regex, Simplified Python and query script in Python'
	},
	'Atualizações': {
		'en-US': 'Updates'
	},
	...
	'Salve a busca para liberar mais opções': {
		'pt-BR': 'Filtrar (salve a busca para liberar)',
		'en-US': 'Filter (save this query to unlock)'
	},
	'Parâmetros:': {
		'en-US': 'Parameters:'
	},
	...
	'Filtrar as frases que foram selecionadas manualmente': {
		'pt-BR': 'Filtrar frases manualmente',
		'en-US': 'Filter sentences manually'
	},
	'Filtrar com nova busca': {
		'en-US': 'Filter with a new query'
	},
	'Expressão de busca (?):<span class="tooltiptext">As sentenças serão removidas da página atual e salvas em uma página separada. Se for do desejo, é possível retornar à busca original posteriormente.</span>': {
	...
	'Utilize "nt", "pt" e "ht" como abreviações para next_token, previous_token e head_token': {
		'en-US': 'Use "nt", "pt" and "ht" as abbreviations for next_token, previous_token and head_token'
	},
	...
	'Qual token você deseja buscar? (escreva em português ou inglês):': {
		'pt-BR': 'Qual token você deseja buscar? (escreva em português ou inglês):',
		'en-US': 'What token do you want to look for? (write in Portuguese or English):'
	},
	'Qual a sua chave da API da OpenAI?': {
		'en-US': 'What is your OpenAI API key?',
		'pt-BR': 'Qual a sua chave da API da OpenAI?'
	},
...
function generateQueryScript(){
	query = prompt(translations["Qual token você deseja buscar? (escreva em português ou inglês):"][userLang])
	if (query && query.trim().length > 0) {
		loadingScreen()
		$.post('../cgi-bin/api.py', {
			'action': 'generate_query_script',
			'query': query
		},
		function(data) {
			// initiate download by script
			// add this in onload event or after the iframe
			document.getElementById('downloadChatGPTQueryScript').click();
			endLoadingScreen()
		},
		"text",
		);
...
function saveOpenAIKey(){
	key = prompt(translations["Qual a sua chave da API da OpenAI?"][userLang])
	if (key && key.trim().length > 0) {
		...
		function(data) {
			$('#savedOpenAIKey').slideToggle(true);
		},
		"text",
		);
	}
...
function atualizar_pesquisa_sel_label(){
	for (g of Array(5).keys()) {
		group = (g+1).toString()
		group_text = $('.nome_pesquisa_sel[group="' + group + '"]').val() 
		if (group_text && group_text.trim().length > 0) {
			$('.cblabel[group="' + group + '"]').text(group_text)
			$('.cblabel[group="' + group + '"],.cb[group="' + group + '"]').show();
		} else {
			$('.cblabel[group="' + group + '"],.cb[group="' + group + '"]').hide();
		}
	}

	$('.cb').unbind('click')
	$('.cblabel').unbind('click')
	$('.cb').on('click', function() {
		group = $(this).attr('group')
		sent_id = $(this).attr('sent_id')
		$('.cb[sent_id="' + sent_id + '"]').not(this).prop('checked', false)
	});

	$('.cblabel').click(function(){
		group = $(this).attr('group')
		sent_id = $(this).attr('sent_id')
		checkbox = $('.cb[group="' + group + '"][sent_id="' + sent_id + '"]')
		checkbox.click()	
	})
}
...
$(document).ready(function(){
    ...
    $('.exportHtml').click(function(){
        ...
        window.open(url + "?corpus=" + encodeURIComponent($("#corpus").text()) + "&params=" + encodeURIComponent($('#expressao').text()) + "&html=" + $('[name=link_interrogatorio]').val() + "&nome=" + $('[name=nome_interrogatorio]').val().replace(/#/%23), "_blank");
    });
    ...
    $('[tab=selectionFilter]').click(function(){
        $('.nome_pesquisa_sel[group="1"]').focus();
    });
...
function filtrar(){
	...
	data.append("action", "filtrar")	
	data.append("pesquisa", $('#pesquisa_filtro').val())
	data.append("nome_pesquisa", $("#nome_pesquisa").val())
	data.append("udoriginal", $('#corpus').text())
	data.append("pesquisa_original", $("#expressao").text())
	data.append("jsonId", $('#jsonId').val())
	data.append("html", $('#apagar_link').val())
	fetch('../../cgi-bin/api.py', {
		method: "POST",
		body: data
	})
...
function excluir_selection() {
	...
	for (g of Array(5).keys()){
		group = (g+1).toString()
		queryName = $('.nome_pesquisa_sel[group="' + group + '"]').val()
		if (queryName.length > 0){
			checkboxes = $('.cb[group=' + group + ']:visible')
			checkboxes.each(function(index){
				if ($(checkboxes[index]).prop('checked') == true) {
					if (!(group in queries)) {
						queries[group] = {'parameters': [], 'queryname': queryName}
					}
					sent_id = $(checkboxes[index]).attr('sent_id')
					tokens = $('.sentenceTokens[sent_id="' + sent_id + '"]').attr('tokens')
					queries[group]['parameters'].push(sent_id + ":" + tokens)
				}
			})
		}
	}

	n_queries = Object.keys(queries).length
	if (n_queries) {
		loadingScreen()
		let data = new FormData()
		data.append("action", "filtrar")
		data.append("pesquisa", "")
		data.append("selections", JSON.stringify(queries))
		data.append("udoriginal", $('#corpus').text())
		data.append("pesquisa_original", $('#expressao').text())
		data.append("jsonId", $('#jsonId').val())
		data.append("html", $('#apagar_link').val())
	...
	/*
    $('#pesquisa_filtro').val('tokens=');
    $(".cb:checked").each(function(){
        $('#pesquisa_filtro').val($('#pesquisa_filtro').val() + escapeRegExp($(this).attr('sent_id')) + ":" + escapeRegExp($(this).attr('tokens')) + '|');
    });

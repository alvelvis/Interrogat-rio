...
if not 'pesquisa' in form and not 'action' in form:
	...
	html += "<title>{title}</title><h1>{nome_filtro} (<span class='len_filtros'>{len_filtros}</span>)</h1>\
		...
		<a href='#' class='translateHtml' onclick='window.close()'>[Fechar página]</a><br><span class='extractSentidSpan' style='display:none'></span><input class='extractSentidInput' style='width: 300px; display:none; margin:5px;'>\
		<br>{parametros}<br><br>\

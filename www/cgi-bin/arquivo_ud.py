...
if os.environ['REQUEST_METHOD'] != 'POST' and not 'validate' in form:
    ...
    if 'win' in sys.platform:
        ...
        html = html.replace('<select id="chooseLanguage" name="chooseLanguage" style="margin-top: 10px; margin-bottom: 10px; display:none">', '<select id="chooseLanguage" name="chooseLanguage" style="margin-top: 10px; margin-bottom: 10px;">')
    else:

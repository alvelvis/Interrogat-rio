...
def renderSentences(script=""):
    ...
    for i, ocorrencia in enumerate(resultados):
        ...
        #arquivoHtml += '<br>'
        arquivoHtml += f"<form action=\"../../cgi-bin/udpipe.py?conllu={conllu}\" target=\"_blank\" method=POST id=udpipe_{str(startPoint+i+1)}><input type=hidden name=textheader value=\"{estruturado.text}\"></form><a style=\"cursor:pointer\" onclick='anotarudpipe(\"udpipe_{str(startPoint+i+1)}\")'>Anotar frase com o UDPipe</a>"
        arquivoHtml += '<br>'

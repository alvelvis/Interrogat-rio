        ...
        if os.path.isfile("../cgi-bin/filtros.json"):
            ...
            if pagina_html in filtros_json:
                ...
                for pagina in [[x, len(filtros_json[pagina_html]['filtros'][x]['sentences'])] for x in filtros_json[pagina_html]['filtros']]:
                    ...
                    pagina_filtros += f'<li><a style="cursor:pointer;" title="Clique para adicionar ao mesmo filtro" target="_blank" href=\'../../cgi-bin/filtrar.cgi?action=view&html={pagina_html}&filtro=' + encodeUrl(pagina[0]) + f'\'>{pagina[0]} ({pagina[1]})</li>'
            else:
                filtros = []

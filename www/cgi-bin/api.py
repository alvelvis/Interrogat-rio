...
def renderSentences(script=""):
        ...
        if os.path.isfile("../cgi-bin/filtros.json"):
            ...
            if pagina_html in filtros_json:
                ...
                for pagina in [[x, len(filtros_json[pagina_html]['filtros'][x]['sentences'])] for x in filtros_json[pagina_html]['filtros']]:
                    filtrar_filtros += f'<li><a style="cursor:pointer;" onclick="$(\'#nome_pesquisa,#nome_pesquisa_sel\').val($(this).children(nome).text());"><span id="nome">{pagina[0]}</a> ({pagina[1]})</li>'
                    pagina_filtros += f'<li><a style="cursor:pointer;" target="_blank" href=\'../../cgi-bin/filtrar.cgi?action=view&html={pagina_html}&filtro=' + web.escape(pagina[0]) + f'\'>{pagina[0]} ({pagina[1]})</li>'
            else:
                filtros = []
    ...
    for i, ocorrencia in enumerate(resultados):
        ...
        if ((estruturado.sent_id and ('-' in estruturado.sent_id or re.search(r'^\d+$', estruturado.sent_id))) or estruturado.id) and estruturado.text:
            arquivoHtml += f"<p><input class='translateVal' id=contexto_{str(startPoint+i+1)} value=\"Mostrar contexto\" onclick=\"contexto('{estruturado.sent_id}', '{estruturado.id}', '{corpus}')\" style=\"margin-left:0px\" type=button> <input class='translateVal anotacao' id=mostrar_{str(startPoint+i+1)} value=\"Mostrar anotação\" onclick=\"mostrar('div_{str(startPoint+i+1)}', 'mostrar_{str(startPoint+i+1)}')\" style=\"margin-left:0px\" type=button> <input class='translateVal opt' id=opt_{str(startPoint+i+1)} value=\"Mostrar opções\" onclick=\"mostraropt('optdiv_{str(startPoint+i+1)}', 'opt_{str(startPoint+i+1)}')\" style=\"margin-left:0px\" type=button> <input class=\"abrirInquerito translateVal\" type=button value=\"Abrir inquérito\" onclick='inquerito(\"form_{str(startPoint+i+1)}\")'></p>" + "\n"
        else:
            arquivoHtml += f"<p><input id=mostrar_{str(startPoint+i+1)} class=\"translateVal anotacao\" value=\"Mostrar anotação\" onclick=\"mostrar('div_{str(startPoint+i+1)}', 'mostrar_{str(startPoint+i+1)}')\" style=\"margin-left:0px\" type=button> <input id=opt_{str(startPoint+i+1)} class=\"translateVal opt\" value=\"Mostrar opções\" onclick=\"mostraropt('optdiv_{str(startPoint+i+1)}', 'opt_{str(startPoint+i+1)}')\" style=\"margin-left:0px\" type=button> <input type=button value=\"Abrir inquérito\" class='translateVal' onclick='inquerito(\"form_{str(startPoint+i+1)}\")'></p>" + '\n'

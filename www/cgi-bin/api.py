...
def renderSentences(script=""):
    ...
    criterio = form['parametros'].value.split(" ", 1)[0]
    ...
    sys.stderr.write(caminhoCompletoConllu + criterio + parametros + script); sys.stderr.flush()
    ...
    sys.stderr.write(str(resultadosBusca)); sys.stderr.flush()

    numeroOcorrencias = len(resultadosBusca['output'])
    if numeroOcorrencias > startPoint + 20:
    ...
    with open("../cgi-bin/apagarApi.html", "w") as f:
        f.write(arquivoHtml)

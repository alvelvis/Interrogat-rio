...
cgitb.enable()
import re, html, time
from utils import prettyDate, encodeUrl
import json
import html as web
			...
			if (coluna != ':' and valor and len(query.split('\t')) >= int(coluna) + 1 and (re.search(valor, query.split('\t')[int(coluna)], flags=re.I|re.M))) or (not coluna) or (coluna == ':' and re.search(valor, query, flags=re.I|re.M)):
				...
				if query.split('\t')[0].rsplit('.html', 1)[0].rsplit("/", 1)[1] in filtros_json:
					...
					for filtro in filtros_json[query.split('\t')[0].rsplit('.html', 1)[0].rsplit("/", 1)[1]]['filtros']:
						filtros += '<a target="_blank" href=\'../cgi-bin/filtrar.py?action=view&html=' + query.split('\t')[0].rsplit('.html', 1)[0].rsplit("/", 1)[1] + '&filtro=' + encodeUrl(filtro) + '\'>' + filtro + ' ('+ str(len(filtros_json[query.split('\t')[0].rsplit('.html', 1)[0].rsplit("/", 1)[1]]['filtros'][filtro]['sentences'])) + ')</a><br>'
				html_query += '''<div class="container-lr"><table style="width:100%"><tr><td style="padding-top:0px; padding-bottom:0px; max-width: 60%; word-wrap: break-word;"><p style="padding-top:0px; padding-bottom:0px; max-width: 60%; word-wrap: break-word;"><h3><a href="../''' + query.split('\t')[0] + '''">''' + web.escape(query.split('\t')[1]) + ''' (''' + query.split('\t')[2] + ''')</a> <!--a class="close-thik" href="#" onclick='apagar("''' + query.split('\t')[0].split('resultados/')[1].split('.html')[0] + '''")'></a--></h3></p></td></tr></table><table style="width:100%"><tr><td colspan="2" style="word-wrap: break-word;"><div style="word-wrap: break-word; display: inline-block;">''' + web.escape(query.split('\t')[4]) + '''</div></td></tr><tr><td style="min-width: 50%; max-width:50%; word-wrap: break-word;"><a href="#" onclick="document.getElementById('coluna').value = '5'; document.getElementById('valor').value = \'''' + query.split('\t')[5] + '''\'; document.getElementById('pesquisa').submit();">''' + query.split('\t')[5] + '''</a><br><small>''' + prettyDate(query.split('\t')[6].replace("_", " ")).beautifyDateDMAH() + '''<br><a style="cursor:pointer;" onclick="if(window.confirm('Deseja apagar a busca &quot;{}&quot;?')) {{ window.location.href = '../cgi-bin/excluir_query.py?html='''.format(query.split("\t")[1].replace('"', "&quot;")) + query.split('\t')[0].rsplit('.html', 1)[0].rsplit("/", 1)[1] + "&json_id=" + query.split("\t")[7] + ''''; }" class="translateHtml">excluir</a></small></td><td style="max-width: 50%; word-wrap: break-word;">''' + filtros + '''</td></tr></table></div>\n'''
				# if conectado else '''<div class="container-lr" ><p><h3><a href="''' + query.split('\t')[0] + '''">''' + web.escape(query.split('\t')[1]) + ''' (''' + query.split('\t')[2] + ''')</a> <!--a class="close-thik" href="#" onclick='apagar("''' + query.split('\t')[0].split('resultados/')[1].split('.html')[0] + '''")'></a--></h3></p><p><div class="tooltip" style="display: inline-block">''' + query.split('\t')[3] + ''' ''' + web.escape(query.split('\t')[4]) + '''<span class="tooltiptext">''' + criterios[int(query.split('\t')[3])].split('<h4>')[0] + '''</span></div></p><small><p>''' + query.split('\t')[5] + '''</p><p>''' + query.split('\t')[6] + '''</p></small></div>\n''' # &nbsp;&nbsp;&nbsp;&nbsp;
				total += 1

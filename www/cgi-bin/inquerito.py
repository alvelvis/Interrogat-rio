...
from utils import corpusGenericoInquerito, fastsearch, cleanEstruturaUD, build_modifications_html, delete_from_conllu_json
	...
	for key in dict(form).keys():
		...
		if re.search(r'^\d+-(\d+|meta)$', key) and not '# sent_id = ' in value.value:
			...
			new_inquiries.append({
				...
				'token_id': conlluzao[sentnum][token][0],
